#include "pch.h"
#include "rpg_game/random.h"
#include "rpg_game/item.h"
#include "rpg_game/item_manager.h"
#include <iostream>
...
void display_character_sheet() {
	...
	if (MainCharacter->us.GetEquippedWeaponAt((unsigned long long)WEAPONSLOT::RANGED)) {
		...
		std::cout << "RANGED: " << weapon_name << "    damage(" << MainCharacter->us.GetEquippedWeaponAt((unsigned long long)WEAPONSLOT::RANGED)->MinDamage << '-' << MainCharacter->us.GetEquippedWeaponAt((unsigned long long)WEAPONSLOT::RANGED)->MinDamage << ")\n";
	}
...
Item* drop_random_item() {
	...
	// HEAD, CHEST, LEGS, FEET, HANDS, RING1, RING2, NECK
	...
	if (drop_seed < 6) {
		...
		return ItemManager::CreateArmor("Helmet", CoreStats(0, 0, 0, 1, 0), ARMORSLOT::HEAD);
	}
...
void fight_sequence(Player& player1) {
	...
	while (player1.IsAlive() && CurrentMonster->IsAlive()) {
		...
		while (action_taken == FightOptions::NONE) {
			...
			std::cout
				...
				<< "action(a: attack, i:inv, b:abilities): ";
		...
		// monster hits when your turn over
	...
	if (player1.IsAlive()) {
		...
		// note monsters defeated count and prepare next one
		monsters_defeated++;
